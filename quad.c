	...
	for (n=0;n<images;n++) {
		...
		for(i=1;i<iters;i++){
			if(mode==1){  // Simple polynomial form
				...
				//~ y[i]=	ay[0] + ay[1]*x[i-1] + ay[2]*y[i-1] + ay[3]*z[i-1] + ay[4]*ffabs(x[i-1]) + ay[5]*ffabs(y[i-1]) + ay[6]*ffabs(z[i-1]);
				//~ z[i]=	az[0] + az[1]*x[i-1] + az[2]*y[i-1] + az[3]*z[i-1] + az[4]*ffabs(x[i-1]) + az[5]*ffabs(y[i-1]) + az[6]*ffabs(z[i-1]);
				//~ 
				//~ xenew=	ax[0] + ax[1]*xe + ax[2]*ye + ax[3]*ze + ax[4]*ffabs(xe) + ax[5]*ffabs(ye) + ax[6]*ffabs(ze);
				//~ yenew=	ax[0] + ax[1]*xe + ax[2]*ye + ax[3]*ze + ax[4]*ffabs(xe) + ax[5]*ffabs(ye) + ax[6]*ffabs(ze);
				//~ zenew=	ax[0] + ax[1]*xe + ax[2]*ye + ax[3]*ze + ax[4]*ffabs(xe) + ax[5]*ffabs(ye) + ax[6]*ffabs(ze);
				
				 // 2D
				x[i]=	ax[0] + ax[1]*x[i-1] + ay[2]*y[i-1] + ax[3]*fabs(x[i-1]) + ax[4]*fabs(y[i-1]);
				y[i]=	ay[0] + ay[1]*x[i-1] + ay[2]*y[i-1] + ay[3]*fabs(x[i-1]) + ay[4]*fabs(y[i-1]);
				
				xenew=	ax[0] + ax[1]*xe + ay[2]*ye + ax[3]*fabs(xe) + ax[4]*fabs(ye);
				yenew=	ay[0] + ay[1]*xe + ay[2]*ye + ay[3]*fabs(xe) + ay[4]*fabs(ye);
			}else if(mode==3){  // Exponential function
			...
			}else if(mode==4){ // SOmething that I dont remember
				...
				yenew=ay[0]+ze-ze*xe;
				zenew=az[0]+xe-xe*ye;
			}else if(mode==5){ // Another one that I also dont remember
			...
			if(mode==1 || mode==2){ // If the attractor is 2d this tests will be used
				/* Update the bounds */
				xmin = MIN(xmin,x[i]);
				ymin = MIN(ymin,y[i]);
				xmax = MAX(xmax,x[i]);
				...
				if (xmin < -1e7 || ymin < -1e7 || xmax > 1e7 || ymax > 1e7) {
					drawit = FALSE;
			...
			}else if(mode==3 || mode==4 || mode==5){ // This one is for 3d attractors. The lyaps may be wrong but it sould work
				/* Update the bounds */
				xmin = MIN(xmin,x[i]);
				ymin = MIN(ymin,y[i]);
				zmin = MIN(zmin,z[i]);
				...
				/* Checks if he series tend to infinity */
				...
				/* Check if the series tend to a point */
				dx = x[i] - x[i-1];
				dy = y[i] - y[i-1];
				dz = z[i] - z[i-1];
